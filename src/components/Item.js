  ...
  return (
    <div className='item-wrapper'>
      ...
      <nav className='item-sub-nav'>
        ...
        <NavLink to={`${itemID}/description`}>Description</NavLink>
        <NavLink to={`${itemID}/shipping`}>Shipping</NavLink>
      </nav>

      {/* 👉 STEP 9 - Here go the Routes for `<current path>/shipping` and `<current path>/description` */}
      {/* These Routes should render <ItemDetails />  */}
      <Routes>
        <Route path={`${itemID}/description`} element={<ItemDetails text={item.description} />} />
        <Route path={`${itemID}/shipping`} element={<ItemDetails text={item.shipping} />} />
      </Routes>
    </div>
  )
}
